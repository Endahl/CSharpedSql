namespace Endahl.CSharpedSql.SqlServer.Base
{
    ...
    public class SqlServerBase : ISqlBase
    {
        ...
        {
            return drop.DropIfTableExists
                ? $"IF(object_id(N'{sql.IdentifieName(drop.TableName)}', N'U') IS NULL BEGIN DROP TABLE {sql.IdentifieName(drop.TableName)}"
        ...
        public virtual string Function(Function function, SqlOptions sql)
        {
            ...
            switch (function.FunctionType)
            {
                ...
                case FunctionType.DateDifference:
                    result = $"DATEDIFF(day, {sql.IdentifieName(function.ColumnName)}, {sql.CreateItemID(function.String)})";
                    break;
                case FunctionType.DateDifferenceColumn:
                    result = $"DATEDIFF(day, {sql.IdentifieName(function.ColumnName)}, {sql.IdentifieName(function.String)})";
                    break;
                ...
                case FunctionType.IndexOf:
                    result = "CHARINDEX";
                    result += $"({sql.CreateItemID(function.String)}, {sql.IdentifieName(function.ColumnName)})";
                    break;
                case FunctionType.Insert:
                    result = $"STUFF({sql.IdentifieName(function.ColumnName)}, {sql.CreateItemID(function.Number)}, {sql.CreateItemID(function.Number2)}, {sql.CreateItemID(function.String)})";
                    break;
                case FunctionType.IsNull:
                    result = $"ISNULL({sql.IdentifieName(function.ColumnName)}, ";
                    if (function.Number != null)
                        result += $"{sql.CreateItemID(function.Number)})";
                    else if (function.Decimal != null)
                        result += $"{sql.CreateItemID(function.Decimal)})";
                ...
                case FunctionType.Length:
                    result = $"LEN({sql.IdentifieName(function.ColumnName)})";
                    break;
                case FunctionType.Locate:
                    result = $"CHARINDEX({sql.CreateItemID(function.String)}, {sql.IdentifieName(function.ColumnName)})";
                    break;
                ...
                case FunctionType.Repeat:
                    result = $"REPLICATE({sql.IdentifieName(function.ColumnName)}, {sql.CreateItemID(function.Number)})";
                    break;
            ...
            else if (select.SelectType == SelectType.Top)
            {
                if (select.Offset > 0)
                {
                    if (select.OrderBy == null)
                        throw new Exception("Can't use Offset then OrderBy is NULL");
                    offset = $" OFFSET {select.Offset} ROWS FETCH NEXT {select.Top} ROWS ONLY";
                }
                else
                    ex = $"TOP {select.Top} ";
            }
